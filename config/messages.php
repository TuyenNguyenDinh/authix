<?php

return [
    'common' => [
        'success' => 'Success',
    ],
    'auth' => [
        'email_required' => 'Email is required',
        'email_email' => 'Email is not valid',
        'email_min' => 'Email must be at least :min characters',
        'email_max' => 'Email must be less than :max characters',
        'email_exists' => 'Email does not exist',
        'password_required' => 'Password is required',
        'password_string' => 'Password must be a string',
        'password_min' => 'Password must be at least :min characters',
        'password_max' => 'Password must be less than :max characters',
        'otp_sent' => 'Otp code sent to your email, please check your email',
        'otp_verified' => 'Otp code verified',
        'otp_required' => 'Otp code is required',
        'otp_string' => 'Otp code must be a string',
        'otp_min' => 'Otp code must be at least :min characters',
        'otp_max' => 'Otp code must be less than :max characters',
        'otp_expired' => 'Otp code has expired',
        'temp_token_required' => 'Temp token is required',
        'temp_token_string' => 'Temp token must be a string',
        'temp_token_exists' => 'Temp token does not exist',
        'logout' => 'Logout successfully',
    ]
];
